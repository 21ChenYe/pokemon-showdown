 ...
 *********************************************************/
 ...
 class BasicRoom {
	...
	roomlog(message) { throw new Error(`should be implemented by subclass`); }
	modlog(message) { throw new Error(`should be implemented by subclass`); }
 ...
 class GlobalRoom extends BasicRoom {
	...
	constructor(roomid) {
		...
		for (const [i, chatRoomData] of this.chatRoomDataList.entries()) {
			...
			const room = exports.Rooms.createChatRoom(id, chatRoomData.title, chatRoomData);
			if (room.aliases) {
 ...
 = {
	...
	addChatRoom(title) {
		...
		const room = exports.Rooms.createChatRoom(id, title, chatRoomData);
	...
	reportCrash(err, crasher = "The server") {
		...
		if (devRoom) {
			...
			if (exports.Rooms.lobby) exports.Rooms.lobby.add(crashMessage).update();
			const staffRoom = exports.Rooms.get('staff');
...
} exports.BasicChatRoom = BasicChatRoom;
...
function getRoom(roomid) {
 ...
 const Rooms = {
	...
	createChatRoom(roomid, title, options) {
		if (exports.Rooms.rooms.has(roomid)) throw new Error(`Room ${roomid} already exists`);
		const room = new BasicChatRoom(roomid, title, options) ;
		exports.Rooms.rooms.set(roomid, room);
		return room;
	},
...
exports.Rooms.global = new GlobalRoom('global' );

exports.Rooms.rooms.set('global' , exports.Rooms.global);
