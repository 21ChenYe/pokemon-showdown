 ...
 */
 ...
 class TeamValidator {
	...
	constructor(format) {
		this.format = _dex.Dex.getFormat(format);
		this.dex = _dex.Dex.forFormat(this.format);
	...
	baseValidateTeam(team, removeNicknames = false) {
		...
		if (['triples', 'rotation'].includes(format.gameType ) && minSize < 3) minSize = 3;
	...
	validateSet(set, teamHas) {
		...
		set.species = _dex.Dex.getSpecies(set.species);
		set.name = dex.getName(set.name);
		let item = dex.getItem(_dex.Dex.getString(set.item));
		set.item = item.name;
		let ability = dex.getAbility(_dex.Dex.getString(set.ability));
		set.ability = ability.name;
		set.nature = dex.getNature(_dex.Dex.getString(set.nature)).name;
		...
		if (ruleTable.has('obtainableformes')) {
			...
			} else if (template.id === 'rayquaza' && set.moves.map(toID).includes('dragonascent' )) {
		...
		for (const moveName of set.moves) {
			...
			const move = dex.getMove(_dex.Dex.getString(moveName));
	...
	validateStats(set, template, setSources) {
		const ruleTable = this.ruleTable;
		const dex = this.dex;
		...
		if (dex.currentMod === 'letsgo') { // AVs
			for (const stat in set.evs) {
				if (set.evs[stat ] > 0 && !allowAVs) {
					...
					problems.push(`${name} has more than 200 Awakening Values in ${statTable[stat ]}.`);
				}
			}
		} else { // EVs
			for (const stat in set.evs) {
				if (set.evs[stat ] > 255) {
					problems.push(`${name} has more than 255 EVs in ${statTable[stat ]}.`);
				}
			}
		}
	 ...
	 */
	validateSource(
		set, source, setSources, template, because
	) {
		let eventData = null;
		let eventTemplate = template;
		if (source.charAt(1) === 'S') {
			const splitSource = source.substr(source.charAt(2) === 'T' ? 3 : 2).split(' ');
			const dex = (this.dex.gen === 1 ? _dex.Dex.mod('gen2') : this.dex);
		...
		return this.validateEvent(set, eventData, eventTemplate, because ) ;
	}

	

	findEggMoveFathers(source, template, setSources, getAll = false) {
	 ...
	 */
	fatherCanLearn(template, moves, eggGen) {
	...
	checkSpecies(set, template, tierTemplate, setHas) {
		const dex = this.dex;
		const ruleTable = this.ruleTable;
		...
		if (tierTemplate.isNonstandard && tierTemplate.isNonstandard !== 'Unobtainable') {
			...
			let isUnreleased = tierTemplate.isUnreleased;
