...
/** A Pokemon that has fainted. */
 ...
 class Battle {
	...
	resetRNG() {
		this.prng = new (0, _prng.PRNG)(this.prng.startingSeed);
	}
	...
	runEvent(
		...
		for (const handler of handlers) {
			...
			if (eventid !== 'Start' && eventid !== 'SwitchIn' && eventid !== 'TakeItem' &&
				status.effectType === 'Item' && (thing instanceof _pokemon.Pokemon) && thing.ignoringItem()) {
				if (eventid !== 'Update') {
					this.debug(eventid + ' handler suppressed by Embargo, Klutz or Magic Room');
				}
				continue;
			} else if (eventid !== 'End' && status.effectType === 'Ability' &&
					(thing instanceof _pokemon.Pokemon) && thing.ignoringAbility()) {
				if (eventid !== 'Update') {
					this.debug(eventid + ' handler suppressed by Gastro Acid');
				}
				continue;
	...
	findEventHandlers(thing, eventName, sourceThing) {
		...
		if (thing instanceof _pokemon.Pokemon && thing.isActive) {
		...
		if (thing instanceof _side.Side) {
...
(
	...
	runMove(
		moveOrMoveName, pokemon, targetLoc,
		sourceEffect, zMove, externalMove,
		maxMove
	) {
	...
	tryPrimaryHitEvent(
		damage, targets, pokemon, move,
		moveData, isSecondary
	) {
